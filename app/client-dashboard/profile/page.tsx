...
export default function ProfilePage() {
  ...
  const handleContactClick = () => {
    //điều hướng về trang chủ và cuộn đến ID contact-section
    router.push("/contact");
  };
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF0] text-stone-800">
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-stone-100">
          ...
          <div className="flex flex-col items-center mb-10">
            <div className="relative w-32 h-32 mb-4 group">
              <div className="w-full h-full rounded-full bg-stone-100 border-2 border-[#BACAF2] overflow-hidden flex items-center justify-center">
                {/* Icon mặc định hoặc ảnh đại diện */}
                <span className="text-stone-400 text-sm">Upload</span>
              </div>

              {/* Input file ẩn để người dùng click vào ảnh là mở thư mục chọn file */}
              <input
                type="file"
              ...
              {/* Overlay khi hover */}
              <div className="absolute inset-0 bg-black/20 rounded-full opacity-0 group-hover:opacity-100 transition flex items-center justify-center text-white text-xs">
                Đổi ảnh
              </div>
            </div>
            <p className="text-sm text-stone-500">Ảnh đại diện của bạn</p>
          </div>
          {/* Tabs */}
          <div className="flex gap-8 border-b border-stone-100 mb-8">
            <button
              onClick={() => setActiveTab("basic")}
              className={`pb-2 ${activeTab === "basic" ? "border-b-2 border-[#BACAF2] text-[#BACAF2]" : "text-stone-400"}`}
            >
              Thông tin cơ bản
            </button>
            <button
              onClick={() => setActiveTab("account")}
              className={`pb-2 ${activeTab === "account" ? "border-b-2 border-[#BACAF2] text-[#BACAF2]" : "text-stone-400"}`}
            >
              Thông tin tài khoản
            </button>
          </div>
          ...
          {activeTab === "basic" ? (
            <div className="grid grid-cols-2 gap-6">
              {/* Để trống các ô như yêu cầu */}
              <input
                className="w-full p-3 bg-stone-50 rounded-lg border border-stone-100"
              ...
              />
              <input
                className="col-span-2 w-full p-3 bg-stone-50 rounded-lg border border-stone-100"
                placeholder="Địa chỉ"
              />
            </div>
          ) : (
            <div className="space-y-6">
              ...
              <div className="flex gap-4">
                <button className="px-6 py-2 border border-[#BACAF2] text-[#BACAF2] rounded-lg">
                  Đổi email
                </button>
                <button className="px-6 py-2 border border-[#BACAF2] text-[#BACAF2] rounded-lg">
                  Đổi mật khẩu
          ...
          <div className="mt-8 flex gap-4">
            <button className="bg-[#BACAF2] text-white px-8 py-2 rounded-lg">
              Lưu thông tin
            </button>
            <button className="border border-stone-200 px-8 py-2 rounded-lg">
              Reset
            </button>
          </div>
        </div>
      ...
      >
        {/* Sửa lại class grid và các class căn lề con */}
        <div className="max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-12 pb-8 px-6">
          {/* Cột 1: Căn trái cố định */}
          <div className="flex flex-col items-center md:items-start text-center md:text-left">
            <h4 className="text-base font-black uppercase tracking-wider text-stone-900 mb-4">
              Chúng Tôi Luôn Ở Đây
            </h4>
            <div className="space-y-2 font-medium text-stone-500 leading-relaxed">
              <p>
                Bạn có câu hỏi? Bạn có thể tìm câu trả lời trong mục Câu hỏi
                thường gặp (FAQs).
              </p>
              <p>Hoặc bạn cũng có thể liên hệ trực tiếp với chúng tôi:</p>
              <p className="font-bold text-stone-800">Điện thoại: 1900 1234</p>
              <p>Thứ Hai đến Thứ Sáu: 8:00 - 17:00</p>
              <p>Thứ Bảy: 9:00 - 18:00</p>
              <p className="text-[#6FAEC0] hover:underline cursor-pointer pt-1 block">
                Gửi email cho chúng tôi
              </p>
            </div>
          </div>

          {/* Cột 2: Căn phải cố định */}
          <div className="flex flex-col items-center md:items-end text-center md:text-right">
            <h4 className="text-base font-black uppercase tracking-wider text-stone-900 mb-4 w-full">
              Dịch Vụ Khách Hàng
            </h4>
            <ul className="space-y-2 font-medium text-stone-500 w-full">
              <li
